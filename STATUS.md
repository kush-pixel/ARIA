...
Previous: 2026-04-26 by Sahil (Phase 5 complete — Fixes 13, 14, 24, 33, 42 L1 implemented; 388 unit tests passing, ruff clean)
...
- backend/app/services/fhir/adapter.py — Phase 1 complete (2026-04-27): Fix 6: _build_simple_observation() + PULSE (8867-4) / WEIGHT (29463-7, lb→kg) / PULSEOXYGEN (59408-5) / TEMP (8310-5) scalar Observations emitted per vitals row alongside BP panel; Fix 9: _build_allergy_intolerances() filters ALLERGY_STATUS != "Active" and captures ALLERGY_REACTION from ALLERGY_DETAIL[0] into reaction[0].manifestation[0].text; Fix 7: _build_problem_assessments() collects per-visit {problem_code, visit_date, htn_flag, status_text, assessment_text, status_flag} from PROBLEM_STATUS2_FLAG + PROBLEM_ASSESSMENT_TEXT, passed as _aria_problem_assessments; Fix 12: _build_visit_dates() collects all 124 ADMIT_DATE values, passed as _aria_visit_dates; Fix 8: _build_social_context() joins SOCIAL_HX_DESCRIPTION + SOCIAL_HX_COMMENT from most recent visit with entries, passed as _aria_social_context; Fix 35: _pseudonymize_patient_id() HMAC-SHA256 helper added (inactive — requires PATIENT_PSEUDONYM_KEY set + DB reset); convert_iemr_to_fhir() accepts optional pseudonym_key param. Observation count: 65 → 190 for patient 1091 (PULSE on 53 BP visits + WEIGHT/SpO2/TEMP where present). AllergyIntolerance: now 6 Active-only (inactive filtered).
- backend/app/services/fhir/ingestion.py — Phase 1 complete (2026-04-27): _get_obs_loinc() + _get_obs_scalar_value() helpers added; first observation pass now routes by LOINC code (55284-4 → BP arrays, 8867-4 → last_clinic_pulse, 29463-7 → last_clinic_weight_kg, 59408-5 → last_clinic_spo2 + historic_spo2); Fix 12: last_visit_date set from max(_aria_visit_dates) when key present, falls back to last BP obs date; Fix 7: problem_assessments read from _aria_problem_assessments; Fix 8: social_context read from _aria_social_context; Fix 9: allergy_reactions[] built parallel to allergies[] from AllergyIntolerance reaction field; Fix 16: recent_labs skeleton (reads _aria_recent_labs if present; NULL for patient 1091 — iEMR has no structured lab observations); readings INSERT loop now pre-filters to BP-panel-only (prevents spurious "missing systolic/diastolic" warnings); all new fields added to cc_values
- backend/app/models/clinical_context.py — 7 new ORM columns added: problem_assessments JSONB, allergy_reactions TEXT[], last_clinic_pulse SMALLINT, last_clinic_weight_kg NUMERIC(5,1), last_clinic_spo2 NUMERIC(4,1), historic_spo2 NUMERIC[], recent_labs JSONB
- scripts/setup_db.py — 7 new ADD COLUMN IF NOT EXISTS migrations; DB-level audit trigger trg_readings_audit on readings table (Fix 38) using three separate asyncpg-compatible execute calls (CREATE FUNCTION, DROP TRIGGER IF EXISTS, CREATE TRIGGER); readings deduplication: 3 duplicate clinic rows (same patient_id+effective_datetime+source) removed before UNIQUE index could be created
...
- backend/app/limiter.py — NEW: shared slowapi Limiter instance (key_func=get_remote_address); imported by main.py and the 3 rate-limited API route files to avoid circular imports (Fix 37)
- backend/app/config.py — two new Settings fields: patient_pseudonym_key: str = "" (HMAC key for Fix 35, inactive until set + DB reset), jwt_expiry_minutes: int = 60 (documents JWT expiry requirement, Fix 36)
- backend/app/main.py — slowapi wired: app.state.limiter = limiter + RateLimitExceeded exception handler (Fix 37)
- backend/app/api/readings.py — @limiter.limit("60/minute") on POST /api/readings; from __future__ import annotations removed (required for slowapi + Pydantic Body() compatibility); Body(...) added to payload param
- backend/app/api/ingest.py — @limiter.limit("5/minute") on POST /api/ingest; from __future__ import annotations removed; Body(...) added to bundle param
- backend/app/api/alerts.py — @limiter.limit("30/minute") on GET /api/alerts
...
- backend/tests/test_fhir_adapter.py — updated 2026-04-27: 35 new tests added (TestBuildVitalObservations × 6, TestAllergyActiveFilerAndReaction × 4, TestBuildProblemAssessments × 7, TestBuildVisitDates × 5, TestBuildSocialContext × 4, TestPseudonymizePatientId × 4, 3 bundle-key presence tests, _allergy() fixture updated with ALLERGY_STATUS="Active"); 77 total unit tests passing
- backend/tests/test_ingestion.py — updated 2026-04-27: 18 new tests added (_get_obs_loinc, _get_obs_scalar_value, last_visit_date from _aria_visit_dates, fallback to obs, problem_assessments, social_context, allergy_reactions, last_clinic_pulse, last_clinic_spo2+historic, non-BP obs not inserted as readings, recent_labs None and populated); _observation_resource() fixture updated with top-level LOINC 55284-4 code (required for _get_obs_loinc routing); 55 total unit tests passing
- Total unit tests: 431 passing (up from 384), 0 failing — verified 2026-04-27; fixture-based, no real DB or API calls; covers all 14 check functions individually + 4 full validate_llm_output integration tests; asyncio mode=auto; tests: 3 PHI leak, 3 prompt injection, 11 guardrail (9 parametrized + 2), 3 sentence count, 4 risk score, 4 adherence language, 3 titration window, 3 urgent flags, 3 overdue labs, 6 problem assessments (empty list, hallucinated condition with non-empty problems, synonym acceptance, known condition passes, no condition mention, grounding via problem_assessments keys), 3 medication hallucination, 4 BP plausibility, 3 contradiction, 4 full pipeline (audit event on pass/fail, first-failed-check ordering, compliant summary)
...
- Items 6, 7, 8, 9, 12 COMPLETE (Phase 1 — Kush, 2026-04-27): see adapter.py + ingestion.py entries above; re-ingestion run completed against live Supabase; clinical_context row verified: last_visit_date=2015-11-24, problem_assessments=221 entries, last_clinic_pulse=71 bpm, last_clinic_weight_kg=76.2 kg, last_clinic_spo2=93.0% (7 historic), allergy_reactions=['?','?','?','Generalized Bad Feeling...','MYALGIAS','LEG CRAMPS'], social_context=CONSULTANTS list
- Item 16 COMPLETE (skeleton — Kush, 2026-04-27): recent_labs JSONB column added to clinical_context; ingestion reads _aria_recent_labs bundle key if present; NULL for patient 1091 (iEMR has no structured LOINC lab observations); column ready for future structured lab data
- Items 35, 36, 37, 38 COMPLETE (Phase 8 — Kush, 2026-04-27): slowapi rate limiting live on 3 endpoints; HMAC pseudonymization function wired (inactive); jwt_expiry_minutes config field added; DB audit trigger trg_readings_audit installed on readings table
- Item 39 (TOTP MFA): manual step only — enable in Supabase Auth dashboard; no code required
...
- clinical_context table (2026-04-27 Phase 1): 7 new columns via ADD COLUMN IF NOT EXISTS migrations:
    problem_assessments JSONB — per-visit physician assessment texts (221 entries for patient 1091); gates Phase 2 full-mode comorbidity threshold
    allergy_reactions TEXT[] — parallel to allergies[]; reaction type per allergy (e.g. "MYALGIAS", "LEG CRAMPS")
    last_clinic_pulse SMALLINT — bpm from most recent clinic visit with PULSE data (71 for patient 1091)
    last_clinic_weight_kg NUMERIC(5,1) — kg from most recent clinic visit with WEIGHT data (76.2 for patient 1091)
    last_clinic_spo2 NUMERIC(4,1) — % from most recent clinic visit with SpO2 data (93.0 for patient 1091)
    historic_spo2 NUMERIC[] — all clinic SpO2 readings chronologically (7 entries for patient 1091)
    recent_labs JSONB — structured lab values by LOINC code; NULL for patient 1091 (iEMR has no structured lab obs)
- readings table (2026-04-27): UNIQUE INDEX idx_readings_patient_datetime_source now in place; 3 pre-existing duplicate clinic rows (same patient_id+effective_datetime+source) removed during migration
- DB trigger (2026-04-27): trg_readings_audit AFTER INSERT on readings — safety-net audit record for direct DB writes; reads aria.actor_id and aria.request_id SET LOCAL session vars (Fix 38)
...
Rate limits enforced (slowapi, 2026-04-27):
| POST /api/readings | 60/minute |
| POST /api/ingest | 5/minute |
| GET /api/alerts | 30/minute |

---

## Known Issues
- Supabase confirmed healthy as of 2026-04-27 — no pausing issue.
