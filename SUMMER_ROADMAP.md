...
**Mitigated within the current plan**

| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| Apple App Store delays or requests IRB/legal documentation for the healthcare app | Medium | High | Run the physician validation study via TestFlight — no App Store approval required. Pre-draft all compliance documentation in Week 11 before submission. App Store approval is not on the critical path for the study. |
...
| Anthropic API spend exceeds $400 estimate due to chatbot iteration during development | Medium | Low | Rate limiting is already implemented, capping chatbot at 30 requests/minute per JWT. Prompt caching (Sprint 4) reduces duplicate Layer 3 calls. Monitor spend weekly. |
| Physician recruitment falls short — fewer than 2 practices agree to participate | Medium | High | Begin outreach in Week 1 alongside critical fixes. Minimum viable study is 1 practice and 5 patients. Target IIT-affiliated clinics first. |
...
**May require additional funding if they occur**

| Risk | Likelihood | Impact | Mitigation |
|---|---|---|---|
| Supabase HIPAA upgrade required if the physician study scales beyond de-identified patients | Low | High | Submit the BAA upgrade request as soon as expansion is confirmed. If this occurs, additional funding of approximately $1,200–$1,800 will be required to cover the Supabase Team plan and BAAs from Railway, Sentry, and Anthropic. |
| SMART on FHIR pulled forward from deferred list — Epic or Cerner sandbox access takes 2–4 weeks to provision | Low | Medium | If SMART on FHIR is pulled forward, submit the Epic developer sandbox application immediately. Synthetic FHIR bundles cover all development in the interim so no sprint is blocked. If expedited access is available, additional fees of approximately $200–$500 may be required. |
| Withings Health Solutions API requires a paid partnership tier for clinical study use | Low | Medium | Withings partnership application submitted in Week 1. If clinical-scale access triggers enterprise pricing, additional funding of approximately $300–$400 will be required. |
| Railway Pro compute limits exceeded under physician validation study load | Low | Medium | Load test Celery workers in Week 3. If limits are exceeded during active study monitoring, additional funding of approximately $50–$100 per month will be required for a Railway plan upgrade. |
| Expo EAS Build free tier exhausted during Sprint 5 | Low | Low | Monitor build count weekly during Weeks 12–13. If the free limit is hit, additional funding of approximately $99 will be required for one month of the Expo Production plan. |
