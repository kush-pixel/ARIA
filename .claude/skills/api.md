...
PATCH /api/patients/{patient_id}/appointment  update next_appointment (Fix 21)
POST  /api/ingest                             FHIR Bundle import
POST  /api/readings                           reading ingestion
GET   /api/briefings/{id}                     latest briefing for patient
GET   /api/alerts[?patient_id=]               unacknowledged alerts; optional patient filter (Fix 24)
POST  /api/alerts/{id}/acknowledge            accepts disposition (agree_acting|agree_monitoring|disagree)
                                              and optional reason_text → alert_feedback table (Fix 42 L1)
